...
export async function generateMetadata({ params }: CandidateProfilePageProps): Promise<Metadata> {
  ...
  const candidateKeywords = [leader.name, leader.title, leader.department, `${leader.name} JnUCSU`, `${leader.title.replace(" - JnUCSU", "")} candidate`, ...leader.tags];
  ...
  return generateSEOMetadata({
    ...
    keywords: combineKeywords(KEYWORDS.general, KEYWORDS.candidates, KEYWORDS.leadership, candidateKeywords),
    image: leader.avatar,
    type: "profile",
    url: `/candidates/${id}`,
  });
...
export default async function CandidateProfilePage({ params }: CandidateProfilePageProps) {
  ...
  return (
    <>
      {/* Structured Data */}
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(candidateStructuredData) }} />
